...
def predict_transform(prediction, inp_dim, anchors, num_classes, CUDA = True):
    ...
    # 每个anchor有（5+num_class类）
    bbox_attrs = 5 + num_classes
    num_anchors = len(anchors)

    # 我们必须通过检测特征图的步幅来划分锚点。不然可能anchor比检测的目标还要大
    ...
    # print('anchors1.shape',anchors.shape)
    anchors = anchors.repeat(grid_size*grid_size, 1).unsqueeze(0)
    # print('anchors2.shape',anchors.shape)
    # print('predicition.shape',prediction.shape)

    # 将锚点应用于边界框的尺寸。每个格子对应检测3个anchors。边界比例*achor的大小
    prediction[:,:,2:4] = torch.exp(prediction[:,:,2:4])*anchors
    #将sigmoid函数应用于类的分数
    prediction[:,:,5: 5 + num_classes] = torch.sigmoid((prediction[:,:, 5 : 5 + num_classes]))

    '''
    在这里要做的最后一件事是将检测图的大小调整为输入图像的大小。
    此处的边界框属性根据要素图（例如，13 x 13）调整大小。
    如果输入图像是416 x 416，我们将属性乘以32或stride变量。
    '''
    prediction[:,:,:4] *= stride
    return prediction
...
# prediction 算法的预测值，confidence类的置信度阈值
# num_classes类别，在这边是80，nms_conf非极大值抑制的门限值
