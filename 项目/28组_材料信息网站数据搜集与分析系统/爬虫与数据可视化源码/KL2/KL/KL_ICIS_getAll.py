# -*- coding: UTF-8 -*-
# version: 1.0.0 2019.10.25 所有PDF下载
...
def download():
    ...
    while 1:
        ...
        try:
            time.sleep(1)
            datePath = browser.find_elements_by_xpath(
                "//*[@id='unified_reports_view_main']/li[2]/ul/li/div[2]/input[@value='PriceReport']/../select[2]/option")
            break
...
def parse(text, fileName):
    ...
    if priceRange.find('-')!=-1:
        priceMin = int(re.compile(r'[1-9]\d*\.\d*|0\.\d*[1-9]|[1-9]\d*').findall(priceRange)[0]);
        priceMax = -int(re.compile(r'-[1-9]\d*\.\d*|-0\.\d*[1-9]|-[1-9]\d*').findall(priceRange)[0]);
        infom = {
            "date": fileTime,
