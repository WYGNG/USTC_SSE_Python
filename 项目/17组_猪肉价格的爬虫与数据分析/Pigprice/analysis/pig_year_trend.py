import sys
sys.path.append("./")
from spider.request_factory import bot, urls
...
df1 = pd.DataFrame([eval(a["all"]),eval(a["province"])],
                    index = [a["yyear"],a["year"]],
                    columns = eval(a["day"]))
df1 = df1.transpose()
# plt.figure()
...
df1.plot(ax = axes[0][0],kind = 'bar',title=a["name"])
# plt.show()
##########################################
province = cities[1]
...
pprint(b)
df2 = pd.DataFrame([eval(b["all"]),eval(b["province"])],
                    index = [b["yyear"],b["year"]],
                    columns = eval(b["day"]))
df2 = df2.transpose()
# plt.subplot(222)
df2.plot(ax = axes[0][1],kind = 'bar',title=b["name"])
# plt.show()
##############################################
province = cities[4]
...
pprint(c)
df3 = pd.DataFrame([eval(c["all"]),eval(c["province"])],
                    index = [c["yyear"],c["year"]],
                    columns = eval(c["day"]))
df3 = df3.transpose()
# plt.subplot(223)
df3.plot(ax = axes[1][0],kind = 'bar',title=c["name"])
# plt.show()
##############################################
province = cities[11]
...
pprint(d)
df4 = pd.DataFrame([eval(d["all"]),eval(d["province"])],
                    index = [d["yyear"],d["year"]],
                    columns = eval(d["day"]))
df4 = df4.transpose()
# plt.subplot(223)
df4.plot(ax = axes[1][1],kind = 'bar',title=d["name"])
# plt.show()
############北京上海广东2018对比
