...
class migu():
			...
			if need_down_list is not None:
				for download_name in need_down_list:
					...
					for quality in qualities:
						...
						if res:
							break
						print('[咪咕]: %s-%s下载失败, 将尝试降低歌曲音质重新下载...' % (download_name, quality.get('formatType', '')))
					if res:
	...
	def __download(self, download_name, download_url, savepath, extension='.mp3'):
		...
		savename = '咪咕音乐_{}'.format(download_name)
		count = 0
		while os.path.isfile(os.path.join(savepath, savename+extension)):
			count += 1
			savename = '咪咕音乐_{}_{}'.format(download_name, count)
		savename += extension
		try:
			print('[咪咕]: 正在下载 --> %s' % savename.split('.')[0])
